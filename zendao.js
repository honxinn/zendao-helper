...
(() => {
  $.noConflict(true)(document).ready(async ($) => {
      ...
      function handleDefaultPath(path) {
          ...
          } else if (!(/misc-checkUpdate|user-login|file-read|execution-task|my-work-task|effort-calendar|my-work-bug|effort-view|task-batchCreate|task-create|task-edit|task-view|task-cancel|user-deny-message-ajaxgetmessage|my-effort-all|effort-edit|effort-view|bug-view|effort-createForObject-bug|task-start|task-finish|task-recordEstimate|task-close|qa/.test(path))) {
      ...
      async function setupWorkHoursOverlay() {
        try {
          ...
          // 检查是否在排除的页面中
          if (/misc-checkUpdate|user-login|file-read|execution-task|my-work-task|effort-calendar|my-work-bug|effort-view|task-batchCreate|task-create|task-edit|task-view|task-cancel|user-deny-message-ajaxgetmessage|my-effort-all|effort-edit|effort-view|bug-view|effort-createForObject-bug|task-start|task-finish|task-recordEstimate|task-close|qa/.test(window.location.pathname)) {
